...
function drawCircle(x, y, r, alpha = 1) {
    ctx.beginPath();
...
function drawLine(x1, y1, x2, y2, alpha = 1, width = 1.2) {
    ctx.beginPath();
    ctx.strokeStyle = `rgba(220,220,220,${alpha})`;
    ctx.lineWidth = width;
    ctx.moveTo(x1, y1);
...
function drawTailSting() {
    ...
    for (let i = tailStart; i < body.length - 1; i++) {
        ...
        drawLine(p.x, p.y - curveLift, next.x, next.y - Math.sin(((i + 1 - tailStart) / (body.length - tailStart)) * Math.PI) * 22, 0.7, 1.1);
    }
